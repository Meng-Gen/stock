...
class FinancialAnalysisYearlySpider(scrapy.Spider):
    name = "FinancialAnalysisYearly"
    custom_settings = {
        'ITEM_PIPELINES': {
    ...
    def start_requests(self):
        ...
        for stock_code in stock_codes:
            url = 'http://jdata.yuanta.com.tw/z/zc/zcr/zcra_{stock_code}.djhtm' \
    ...
    def parse(self, response):
        ...
        XPATH_ROOT = '//*[@id="SysJustIFRAMEDIV"]/table/tr/td/form/table/tr/td/table/tr[not(@id="oScrollHead")]'
        rows = response.xpath(XPATH_ROOT)

        # The first row is empty and the second row is sub-header. Therefore we
        # skip it. Then we begin to parse the third row. It is the content of
        # statement dates. The first entry is the name and then the following
        # entries are statement dates.
        name_and_statement_dates = rows[2].xpath('td//text()').extract()
        ...
        for i in range(3, len(rows)):
            name_and_values = rows[i].xpath('td[not(@class="t2")]//text()').extract()
